...
* <span data-ttu-id="c0a6f-115">Microsoft 공간 소리를 사용 하도록 개발 환경을 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-115">Configure your development environment to use Microsoft Spatial Sound.</span></span>
...
### <a name="project-files"></a><span data-ttu-id="c0a6f-133">프로젝트 파일</span><span class="sxs-lookup"><span data-stu-id="c0a6f-133">Project files</span></span>
...
### <a name="objectives"></a><span data-ttu-id="c0a6f-147">목표</span><span class="sxs-lookup"><span data-stu-id="c0a6f-147">Objectives</span></span>
...
<span data-ttu-id="c0a6f-167">이제 Unity에서 프로젝트를 빌드하고 Visual Studio에서 솔루션을 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-167">We will now build the project in Unity and configure the solution in Visual Studio.</span></span>
...
<span data-ttu-id="c0a6f-189">모던 헤드셋에 배포 하는 경우:</span><span class="sxs-lookup"><span data-stu-id="c0a6f-189">If deploying to an immersive headset:</span></span>
...
<span data-ttu-id="c0a6f-205">다음 지침에서는 spatialized 소리를 홀로그램에 연결 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-205">The following instructions will attach a spatialized sound to a hologram.</span></span>
...
#### <a name="instructions"></a><span data-ttu-id="c0a6f-225">지침</span><span class="sxs-lookup"><span data-stu-id="c0a6f-225">Instructions</span></span>
...
<span data-ttu-id="c0a6f-243">제스처 소리 처리기는 다음 작업을 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-243">Gesture Sound Handler performs the following tasks:</span></span>
...
## <a name="chapter-3---spatial-sound-and-spatial-mapping"></a><span data-ttu-id="c0a6f-265">3 장-공간 소리 및 공간 매핑</span><span class="sxs-lookup"><span data-stu-id="c0a6f-265">Chapter 3 - Spatial Sound and Spatial Mapping</span></span>
...
* <span data-ttu-id="c0a6f-278">**소리 및 작업을 사용 하 여 이동 합니다**.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-278">In **Tap To Place With Sound and Action**:</span></span>
  ...
  * <span data-ttu-id="c0a6f-282">**픽업 동작** 및 **배치 동작** 양쪽의 오른쪽 아래에 있는 +를 누릅니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-282">Press the + in the bottom right under both **On Pickup Action** and **On Placement Action**.</span></span> <span data-ttu-id="c0a6f-283">EnergyHub를 장면에서 **없음 (개체)** 필드로 끕니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-283">Drag EnergyHub from the scene into the **None (Object)** fields.</span></span>
    ...
    * <span data-ttu-id="c0a6f-285">**배치 동작에서**   ->  **EnergyHubBase**  ->  **onselect** 함수를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-285">Under **On Placement Action**, click on **No Function** -> **EnergyHubBase** -> **OnSelect**.</span></span>
...
* <span data-ttu-id="c0a6f-302">발견 된 각 **IAudioInfluencer** 에 대해 **applyeffect** 메서드를 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-302">For each **IAudioInfluencer** encountered, calls the **ApplyEffect** method.</span></span>
...
* <span data-ttu-id="c0a6f-320">**계층** 패널에서 **SpatialMapping** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-320">In the **Hierarchy** panel, select **SpatialMapping**.</span></span>
...
* <span data-ttu-id="c0a6f-354">효과적인 사운드 디자인에 대 한 고려 사항을 이해 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-354">Understand considerations for effective sound design.</span></span>
...
#### <a name="target-your-mix-for-70-volume-on-the-hololens"></a><span data-ttu-id="c0a6f-386">HoloLens의 70% 볼륨에 대 한 조합 대상</span><span class="sxs-lookup"><span data-stu-id="c0a6f-386">Target your mix for 70% volume on the HoloLens</span></span>
...
#### <a name="building-your-mix"></a><span data-ttu-id="c0a6f-402">조합 빌드</span><span class="sxs-lookup"><span data-stu-id="c0a6f-402">Building your mix</span></span>
...
<span data-ttu-id="c0a6f-418">공간 사운드를 사용 하는 경우 10-12는 CPU의 약 12%를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-418">When using Spatial Sound, 10 - 12 emitters will consume approximately 12% of the CPU.</span></span>
...
<span data-ttu-id="c0a6f-420">오디오 데이터는 특히 일반적인 샘플 요금 (44.1 및 48 kHz)에서 클 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-420">Audio data can be large, especially at common sample rates (44.1 and 48 kHz).</span></span> <span data-ttu-id="c0a6f-421">일반적인 규칙은 응용 프로그램 메모리 사용을 줄이기 위해 5-10 초 보다 긴 오디오 파일을 스트리밍하는 것입니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-421">A general rule is that audio files longer than 5 - 10 seconds should be streamed to reduce application memory usage.</span></span>

<span data-ttu-id="c0a6f-422">Unity에서는 파일의 가져오기 설정에서 스트리밍을 위해 오디오 파일을 표시할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-422">In Unity, you can mark an audio file for streaming in the file's import settings.</span></span>

![오디오 가져오기 설정](images/audioimportsettings.png)

## <a name="chapter-5---special-effects"></a><span data-ttu-id="c0a6f-424">5 장-특수 효과</span><span class="sxs-lookup"><span data-stu-id="c0a6f-424">Chapter 5 - Special Effects</span></span>

### <a name="objectives"></a><span data-ttu-id="c0a6f-425">목표</span><span class="sxs-lookup"><span data-stu-id="c0a6f-425">Objectives</span></span>

* <span data-ttu-id="c0a6f-426">"Magic Windows"에 깊이를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-426">Add depth to "Magic Windows".</span></span>
* <span data-ttu-id="c0a6f-427">사용자를 가상 세계에 가져옵니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-427">Bring the user into the virtual world.</span></span>

### <a name="magic-windows"></a><span data-ttu-id="c0a6f-428">매직 창</span><span class="sxs-lookup"><span data-stu-id="c0a6f-428">Magic Windows</span></span>

#### <a name="key-concepts"></a><span data-ttu-id="c0a6f-429">주요 개념</span><span class="sxs-lookup"><span data-stu-id="c0a6f-429">Key Concepts</span></span>

* <span data-ttu-id="c0a6f-430">숨겨진 세계에 뷰를 만들면 시각적으로 매력적인 것입니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-430">Creating views into a hidden world, is visually compelling.</span></span>
* <span data-ttu-id="c0a6f-431">홀로그램 또는 사용자가 숨겨진 세계 가까이 있을 때 오디오 효과를 추가 하 여 현실감을 향상 시킵니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-431">Enhance realism by adding audio effects when a hologram or the user is near the hidden world.</span></span>

#### <a name="instructions"></a><span data-ttu-id="c0a6f-432">지침</span><span class="sxs-lookup"><span data-stu-id="c0a6f-432">Instructions</span></span>

* <span data-ttu-id="c0a6f-433">**계층** 패널에서 **HologramCollection** 를 확장 하 고 지 각 **를 선택 합니다**.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-433">In the **Hierarchy** panel, expand **HologramCollection** and select **Underworld**.</span></span>
* <span data-ttu-id="c0a6f-434">지 각 **를 확장 하** 고 **VoiceSource** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-434">Expand **Underworld** and select **VoiceSource**.</span></span>
* <span data-ttu-id="c0a6f-435">**검사기** 패널에서 **구성 요소 추가** 를 클릭 하 고 **사용자 음성 효과** 를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-435">In the **Inspector** panel, click **Add Component** and add **User Voice Effect**.</span></span>

<span data-ttu-id="c0a6f-436">**VoiceSource** 에 **오디오 원본** 구성 요소가 추가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-436">An **AudioSource** component will be added to **VoiceSource**.</span></span>

* <span data-ttu-id="c0a6f-437">**오디오 원본** 에서 **출력** 을 **UserVoice (혼합기)** 로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-437">In **AudioSource**, set **Output** to **UserVoice (Mixer)**.</span></span>
* <span data-ttu-id="c0a6f-438">**Spatialize** 확인란을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-438">Check the **Spatialize** checkbox.</span></span>
* <span data-ttu-id="c0a6f-439">**공간 Blend** 슬라이더를 **3d** 로 끌어 놓거나 편집 상자에 **1** 을 입력 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-439">Drag the **Spatial Blend** slider all the way to **3D**, or enter **1** in the edit box.</span></span>
* <span data-ttu-id="c0a6f-440">**3D 소리 설정** 을 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-440">Expand **3D Sound Settings**.</span></span>
* <span data-ttu-id="c0a6f-441">**Doppler Level** 을 **0** 으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-441">Set **Doppler Level** to **0**.</span></span>
* <span data-ttu-id="c0a6f-442">**사용자 음성 효과** 에서 **부모 개체** 를 장면의 지 수로 **설정 합니다.**</span><span class="sxs-lookup"><span data-stu-id="c0a6f-442">In **User Voice Effect**, set **Parent Object** to the **Underworld** from the scene.</span></span>
* <span data-ttu-id="c0a6f-443">**최대 거리** 를 **1** 로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-443">Set **Max Distance** to **1**.</span></span>

<span data-ttu-id="c0a6f-444">**최대 거리** 를 설정 하면 효과를 사용 하도록 설정 하기 전에 사용자에 게 부모 개체에 대 한 사용자 **음성 효과** 를 알려 줍니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-444">Setting **Max Distance** tells **User Voice Effect** how close the user must be to the parent object before the effect is enabled.</span></span>

* <span data-ttu-id="c0a6f-445">**사용자 음성 효과** 에서 **코러스 매개 변수** 를 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-445">In **User Voice Effect**, expand **Chorus Parameters**.</span></span>
* <span data-ttu-id="c0a6f-446">**깊이** 를 **0.1** 으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-446">Set **Depth** to **0.1**.</span></span>
* <span data-ttu-id="c0a6f-447">**탭 1 볼륨** 을 설정 하 고 **2 볼륨을 탭** 한 다음 **볼륨 3** 개를 **0.8** 으로 탭 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-447">Set **Tap 1 Volume**, **Tap 2 Volume** and **Tap 3 Volume** to **0.8**.</span></span>
* <span data-ttu-id="c0a6f-448">**원본 사운드 볼륨** 을 **0.5** 으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-448">Set **Original Sound Volume** to **0.5**.</span></span>

<span data-ttu-id="c0a6f-449">이전 설정은 사용자의 음성에 다양 한 다양성을 추가 하는 데 사용 되는 Unity **AudioChorusFilter** 의 매개 변수를 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-449">The previous settings configure the parameters of the Unity **AudioChorusFilter** used to add richness to the user's voice.</span></span>

* <span data-ttu-id="c0a6f-450">**사용자 음성 효과** 에서 **Echo 매개 변수** 를 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-450">In **User Voice Effect**, expand **Echo Parameters**.</span></span>
* <span data-ttu-id="c0a6f-451">**지연 시간** 을 **300** 으로 설정</span><span class="sxs-lookup"><span data-stu-id="c0a6f-451">Set **Delay** to **300**</span></span>
* <span data-ttu-id="c0a6f-452">**감소 비율** 을 **0.2** 로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-452">Set **Decay Ratio** to **0.2**.</span></span>
* <span data-ttu-id="c0a6f-453">**원래 사운드 볼륨** 을 **0** 으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-453">Set **Original Sound Volume** to **0**.</span></span>

<span data-ttu-id="c0a6f-454">이전 설정은 사용자의 음성 에코를 발생 시키는 데 사용 되는 Unity **AudioEchoFilter** 의 매개 변수를 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-454">The previous settings configure the parameters of the Unity **AudioEchoFilter** used to cause the user's voice to echo.</span></span>

<span data-ttu-id="c0a6f-455">사용자 음성 효과 스크립트는 다음을 담당 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-455">The User Voice Effect script is responsible for:</span></span>

* <span data-ttu-id="c0a6f-456">사용자와 스크립트가 연결 된 **GameObject** 간의 거리를 측정 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-456">Measuring the distance between the user and the **GameObject** to which the script is attached.</span></span>
* <span data-ttu-id="c0a6f-457">사용자가 **GameObject** 를 연결 하는지 여부를 확인 하는 중입니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-457">Determining whether or not the user is facing the **GameObject**.</span></span>

<span data-ttu-id="c0a6f-458">효과를 사용 하도록 설정 하려면 거리에 관계 없이 사용자가 GameObject 연결 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-458">The user must be facing the GameObject, regardless of distance, for the effect to be enabled.</span></span>

* <span data-ttu-id="c0a6f-459">**AudioChorusFilter** 및 **AudioEchoFilter** 를 **오디오 소스** 에 적용 하 고 구성 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-459">Applying and configuring an **AudioChorusFilter** and an **AudioEchoFilter** to the **AudioSource**.</span></span>
* <span data-ttu-id="c0a6f-460">필터를 사용 하지 않도록 설정 하 여 효과를 비활성화 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-460">Disabling the effect by disabling the filters.</span></span>

<span data-ttu-id="c0a6f-461">사용자 음성 효과는 [MixedRealityToolkit For unity](https://github.com/Microsoft/MixedRealityToolkit-Unity)에서 Mic 스트림 선택기 구성 요소를 사용 하 여 고품질의 음성 스트림을 선택 하 고 Unity의 오디오 시스템으로 라우팅합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-461">User Voice Effect uses the Mic Stream Selector component, from the [MixedRealityToolkit for Unity](https://github.com/Microsoft/MixedRealityToolkit-Unity), to select the high quality voice stream and route it into Unity's audio system.</span></span>

* <span data-ttu-id="c0a6f-462">**계층** 패널에서 **관리자** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-462">In the **Hierarchy** panel, select **Managers**.</span></span>
* <span data-ttu-id="c0a6f-463">**검사기** 패널에서 **Speech Input Handler** 를 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-463">In the **Inspector** panel, expand **Speech Input Handler**.</span></span>
* <span data-ttu-id="c0a6f-464">**음성 입력 처리기** 에서 지 이상 **표시** 를 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-464">In **Speech Input Handler**, expand **Show Underworld**.</span></span>
* <span data-ttu-id="c0a6f-465">**No 함수** 를 UnderworldBase로 변경 합니다 **.**</span><span class="sxs-lookup"><span data-stu-id="c0a6f-465">Change **No Function** to **UnderworldBase.OnEnable**.</span></span>

![키워드: 지 각 표시](images/showunderworld.png)

* <span data-ttu-id="c0a6f-467">지 각 **지를 확장** 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-467">Expand **Hide Underworld**.</span></span>
* <span data-ttu-id="c0a6f-468">UnderworldBase **함수** 를 변경 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-468">Change **No Function** to **UnderworldBase.OnDisable**.</span></span>

![키워드: 지 각을 숨깁니다.](images/hideunderworld.png)

#### <a name="build-and-deploy"></a><span data-ttu-id="c0a6f-470">빌드 및 배포</span><span class="sxs-lookup"><span data-stu-id="c0a6f-470">Build and Deploy</span></span>

* <span data-ttu-id="c0a6f-471">이전과 마찬가지로 Unity에서 프로젝트를 빌드하고 Visual Studio에서 배포 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-471">As before, build the project in Unity and deploy in Visual Studio.</span></span>

<span data-ttu-id="c0a6f-472">응용 프로그램을 배포한 후:</span><span class="sxs-lookup"><span data-stu-id="c0a6f-472">After the application is deployed:</span></span>

* <span data-ttu-id="c0a6f-473">표면 (벽, 바닥, 테이블)을 표면에 표시 하 고 "지 각"을 *표시* 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-473">Face a surface (wall, floor, table) and say *"Show Underworld"*.</span></span>

<span data-ttu-id="c0a6f-474">지 수를 표시 하 고 다른 모든 holograms를 숨깁니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-474">The underworld will be shown and all other holograms will be hidden.</span></span> <span data-ttu-id="c0a6f-475">지 수가 표시 되지 않으면 실제 화면을 향하고 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-475">If you do not see the underworld, ensure that you are facing a real-world surface.</span></span>

* <span data-ttu-id="c0a6f-476">1 미터 내에서의 접근 방식으로 통신을 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-476">Approach within 1 meter of the underworld hologram and start talking.</span></span>

<span data-ttu-id="c0a6f-477">이제 오디오 효과가 음성에 적용 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-477">There are now audio effects applied to your voice!</span></span>

* <span data-ttu-id="c0a6f-478">지 수를 끄고 효과가 더 이상 적용 되지 않는 방식을 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-478">Turn away from the underworld and notice how the effect is no longer applied.</span></span>
* <span data-ttu-id="c0a6f-479">지 수를 숨기려면 "지 수 *숨기기"* 라고 표시 합니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-479">Say *"Hide Underworld"* to hide the underworld.</span></span>

<span data-ttu-id="c0a6f-480">지 수는 숨겨지고 이전에 숨겨진 holograms 다시 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-480">The underworld will be hidden and the previously hidden holograms will reappear.</span></span>

## <a name="the-end"></a><span data-ttu-id="c0a6f-481">끝</span><span class="sxs-lookup"><span data-stu-id="c0a6f-481">The End</span></span>

<span data-ttu-id="c0a6f-482">축하합니다!</span><span class="sxs-lookup"><span data-stu-id="c0a6f-482">Congratulations!</span></span> <span data-ttu-id="c0a6f-483">이제 **MR 공간 220: 공간 소리** 를 완료 했습니다.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-483">You have now completed **MR Spatial 220: Spatial sound**.</span></span>

<span data-ttu-id="c0a6f-484">세계를 듣고 경험을 바탕으로 경험을 해보세요.</span><span class="sxs-lookup"><span data-stu-id="c0a6f-484">Listen to the world and bring your experiences to life with sound!</span></span>