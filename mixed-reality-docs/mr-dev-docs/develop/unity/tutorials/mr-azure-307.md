...
## <a name="before-you-start"></a><span data-ttu-id="e4b82-147">시작하기 전에</span><span class="sxs-lookup"><span data-stu-id="e4b82-147">Before you start</span></span>
...
4.  <span data-ttu-id="e4b82-159">**저장소 계정 만들기** 패널에서 다음을 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-159">In the **Create Storage Account** panel:</span></span>
    ...
    6.  <span data-ttu-id="e4b82-165">**보안 전송을** **사용 하지 않도록 설정** 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-165">Leave **Secure transfer required** as **Disabled** .</span></span>
...
3.  <span data-ttu-id="e4b82-185">**만들기** 를 클릭 하면 새 **Machine Learning Studio 서비스** 에 대 한 몇 가지 세부 정보를 제공 해야 하는 창이 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-185">Once you have clicked on **Create** , a panel will appear where you need to provide some details about your new **Machine Learning Studio service** :</span></span>
    ...
    6.  <span data-ttu-id="e4b82-197">드롭다운 메뉴에서 적절 한 **작업 영역 가격 책정 계층** 을 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-197">Select the appropriate **Workspace pricing tier** for you, from the dropdown menu.</span></span>
...
15. <span data-ttu-id="e4b82-276">**학습 모델** 의 아래쪽에서 **점수 모델** 의 왼쪽 위 입력에 출력을 연결 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-276">Connect the output, from the bottom of the **Train Model** , to the top-left input of the **Score Model** .</span></span>
...
6. <span data-ttu-id="e4b82-349">이 패널에서 몇 가지 설정을 확인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-349">In this panel, a few settings need to be verified:</span></span>
...
<span data-ttu-id="e4b82-368">이 패키지를 가져오려면:</span><span class="sxs-lookup"><span data-stu-id="e4b82-368">To import this package:</span></span>
...
<span data-ttu-id="e4b82-386">이를 수행하려면:</span><span class="sxs-lookup"><span data-stu-id="e4b82-386">To do so:</span></span>
...
4.  <span data-ttu-id="e4b82-398">Unity 편집기로 돌아가서 **ShelfKeeper** 클래스가 아래와 동일한 지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-398">Back in the Unity Editor, check that the **ShelfKeeper** class looks like the below:</span></span>
    ...
    > 2. <span data-ttu-id="e4b82-405">**계층 구조** 내에서 해당 개체 옆에 있는 화살표를 마우스 왼쪽 단추를 클릭 하 여 **시간 표시** 개체를 확장 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-405">Within the **Hierarchy** expand the **Time Display** object (by left-clicking the arrow beside it).</span></span> <span data-ttu-id="e4b82-406">다음으로, **검사기** 가 해당 정보를 표시 하도록 **계층 구조** 내에서 ***주 카메라*** 를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-406">Next click the ***Main Camera*** from within the **Hierarchy** , so that the **Inspector** shows its information.</span></span>
...
-   <span data-ttu-id="e4b82-414">현재 날짜 및 시간을 제공 하 여 **Machine Learning 서비스** 인스턴스를 쿼리 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-414">Querying the **Machine Learning Service** instance, providing the current date and time.</span></span>

-   <span data-ttu-id="e4b82-415">JSON 응답을 사용 가능한 데이터로 deserialize 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-415">Deserializing the JSON response into usable data.</span></span>

-   <span data-ttu-id="e4b82-416">데이터 해석, 3 개의 권장 제품 검색</span><span class="sxs-lookup"><span data-stu-id="e4b82-416">Interpreting the data, retrieving the 3 recommended products.</span></span>

-   <span data-ttu-id="e4b82-417">**ShelfKeeper** 클래스 메서드를 호출 하 여 장면의 데이터를 표시 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-417">Calling the **ShelfKeeper** class methods to display the data in the Scene.</span></span>

<span data-ttu-id="e4b82-418">이 클래스를 만들려면:</span><span class="sxs-lookup"><span data-stu-id="e4b82-418">To create this class:</span></span>

1.  <span data-ttu-id="e4b82-419">**프로젝트 패널** 의 **스크립트** 폴더로 이동 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-419">Go to the **Scripts** folder, in the **Project Panel** .</span></span>

2.  <span data-ttu-id="e4b82-420">폴더 내부를 마우스 오른쪽 단추로 클릭 **Create** 하 고  >  **c # 스크립트** 를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-420">Right-click inside the folder, **Create** > **C# Script** .</span></span> <span data-ttu-id="e4b82-421">스크립트 **제품 예측** 을 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-421">Call the script **ProductPrediction** .</span></span>

3.  <span data-ttu-id="e4b82-422">새 **제품 예측** 스크립트를 두 번 클릭 하 여 **Visual Studio 2017** 에서 엽니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-422">Double click on the new **ProductPrediction** script to open it with **Visual Studio 2017** .</span></span>

4.  <span data-ttu-id="e4b82-423">**파일 수정이 검색 되었습니다** . 대화 상자가 나타나면 \* **솔루션 다시 로드** 를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-423">If the **File Modification Detected** dialog pops up, click \* **Reload Solution** .</span></span>

5.  <span data-ttu-id="e4b82-424">제품 예측 클래스의 맨 위에 다음 네임 스페이스를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-424">Add the following namespaces to the top of the ProductPrediction class:</span></span>
...
6.  <span data-ttu-id="e4b82-425">**제품 예측** 클래스 내에서 여러 중첩 클래스로 구성 된 다음 두 개체를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-425">Inside the **ProductPrediction** class insert the following two objects which are composed of a number of nested classes.</span></span> <span data-ttu-id="e4b82-426">이러한 클래스는 Machine Learning 서비스에 대 한 JSON을 serialize 및 deserialize 하는 데 사용 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-426">These classes are used to serialize and deserialize the JSON for the Machine Learning Service.</span></span>
...
7.  <span data-ttu-id="e4b82-427">그런 다음 앞의 코드 위에 다음 변수를 추가 합니다. JSON 관련 코드는 스크립트의 맨 아래, 다른 모든 코드 아래, 그 밖의 코드 아래에 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-427">Then add the following variables above the previous code (so that the JSON related code is at the bottom of the script, below all other code, and out of the way):</span></span>
    ...
    > [!IMPORTANT]
    > <span data-ttu-id="e4b82-428">Machine Learning 포털에서 여기에 있는 **기본 키** 와 **요청-응답 끝점** 을 변수에 삽입 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-428">Make sure to insert the **primary key** and **request-response endpoint** , from the Machine Learning Portal, into the variables here.</span></span> <span data-ttu-id="e4b82-429">아래 이미지는에서 키와 끝점을 가져온 위치를 보여 줍니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-429">The below images show where you would have taken the key and endpoint from.</span></span> 
    >  
...
8.  <span data-ttu-id="e4b82-432">**Start ()** 메서드 내에이 코드를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-432">Insert this code within the **Start()** method.</span></span> <span data-ttu-id="e4b82-433">**Start ()** 메서드는 클래스가 초기화 될 때 호출 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-433">The **Start()** method is called when the class initializes:</span></span>
...
9.  <span data-ttu-id="e4b82-434">다음은 Windows에서 날짜 및 시간을 수집 하 고이를 Machine Learning 실험에서 테이블에 저장 된 데이터와 비교 하는 데 사용할 수 있는 형식으로 변환 하는 메서드입니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-434">The following is the method that collects the date and time from Windows and converts it into a format that our Machine Learning Experiment can use to compare with the data stored in the table.</span></span>
...
10. <span data-ttu-id="e4b82-435">이 클래스는 사용 하지 않으므로 **Update ()** 메서드를 **삭제할** 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-435">You can **delete** the **Update()** method since this class will not use it.</span></span>

11. <span data-ttu-id="e4b82-436">현재 날짜와 시간을 Machine Learning 끝점에 전달 하 고 JSON 형식으로 응답을 수신 하는 다음 메서드를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-436">Add the following method which will communicate the current date and time to the Machine Learning endpoint and receive a response in JSON format.</span></span>
...
12. <span data-ttu-id="e4b82-437">JSON 응답을 deserialize 하 고 deserialization 결과를 **ShelfKeeper** 클래스에 전달 하는 다음 메서드를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-437">Add the following method, which is responsible for deserializing the JSON response, and communicating the result of the deserialization to the **ShelfKeeper** class.</span></span> <span data-ttu-id="e4b82-438">이 결과는 현재 날짜 및 시간에 가장을 판매 하기 위해 예측 된 세 항목의 이름입니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-438">This result will be the names of the three items predicted to sell the most at current date and time.</span></span> <span data-ttu-id="e4b82-439">이전 메서드 아래의 **제품 예측** 클래스에 아래 코드를 삽입 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-439">Insert the code below into the **ProductPrediction** class, below the previous method.</span></span>
...
13. <span data-ttu-id="e4b82-440">**Visual Studio** 를 저장 하 고 **Unity** 에 다시 헤드를 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-440">Save **Visual Studio** and head back to **Unity** .</span></span>

14. <span data-ttu-id="e4b82-441">**스크립트** 폴더의 **제품 예측** 클래스 스크립트를 **기본 카메라** 개체로 끌어 놓습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-441">Drag the **ProductPrediction** class script from the **Script** folder, onto the **Main Camera** object.</span></span>

15. <span data-ttu-id="e4b82-442">장면 및 프로젝트 파일 저장 **File**  >  **장면/파일**  >  **저장 프로젝트** 를 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-442">Save your scene and project **File** > **Save Scene/File** > **Save Project** .</span></span>

## <a name="chapter-10---build-the-uwp-solution"></a><span data-ttu-id="e4b82-443">10 장-UWP 솔루션 빌드</span><span class="sxs-lookup"><span data-stu-id="e4b82-443">Chapter 10 - Build the UWP Solution</span></span>

<span data-ttu-id="e4b82-444">이제 프로젝트가 독립 실행형 응용 프로그램으로 실행 될 수 있도록 프로젝트를 UWP 솔루션으로 빌드할 시간입니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-444">It is now time to build your project as a UWP solution, so that it can run as a standalone application.</span></span>

<span data-ttu-id="e4b82-445">빌드:</span><span class="sxs-lookup"><span data-stu-id="e4b82-445">To Build:</span></span>

1.  <span data-ttu-id="e4b82-446">**파일**  >  **저장 장면을** 클릭 하 여 현재 장면을 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-446">Save the current scene by clicking on **File** > **Save Scenes** .</span></span>

2.  <span data-ttu-id="e4b82-447">**파일**  >  **빌드 설정** 으로 이동</span><span class="sxs-lookup"><span data-stu-id="e4b82-447">Go to **File** > **Build Settings**</span></span>

3.  <span data-ttu-id="e4b82-448">**Unity c # 프로젝트** 라는 상자를 선택 합니다 .이는 빌드를 완료 한 후 클래스를 편집할 수 있도록 하기 때문에 중요 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-448">Check the box called **Unity C# Projects** (this is important because it will allow you to edit the classes after build is completed).</span></span>

4.  <span data-ttu-id="e4b82-449">열려 있는 **장면 추가** 를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-449">Click on **Add Open Scenes** ,</span></span>

5.  <span data-ttu-id="e4b82-450">**빌드** 를 클릭한 다음</span><span class="sxs-lookup"><span data-stu-id="e4b82-450">Click **Build** .</span></span>

    ![UWP 솔루션 빌드](images/AzureLabs-Lab7-54.png)

6.  <span data-ttu-id="e4b82-452">솔루션을 빌드하는 데 사용할 폴더를 선택 하 라는 메시지가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-452">You will be prompted to select the folder where you want to build the Solution.</span></span>

7.  <span data-ttu-id="e4b82-453">**빌드** 폴더를 만들고 해당 폴더 내에서 원하는 적절 한 이름을 사용 하 여 다른 폴더를 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-453">Create a **BUILDS** folder and within that folder create another folder with an appropriate name of your choice.</span></span>

8.  <span data-ttu-id="e4b82-454">새 폴더를 클릭 한 다음 **폴더 선택** 을 클릭 하 여 해당 위치에서 빌드를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-454">Click your new folder and then click **Select Folder** , to begin the build at that location.</span></span>

    ![UWP 솔루션 빌드](images/AzureLabs-Lab7-55.png)

    ![UWP 솔루션 빌드](images/AzureLabs-Lab7-56.png)

9.  <span data-ttu-id="e4b82-457">Unity가 빌드를 완료 하면 (시간이 걸릴 수 있음) 빌드 위치에서 **파일 탐색기** 창이 열립니다. (작업 표시줄은 항상 창 위에 표시 되는 것은 아니지만 새 창 추가를 알려 줍니다.)</span><span class="sxs-lookup"><span data-stu-id="e4b82-457">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-11---deploy-your-application"></a><span data-ttu-id="e4b82-458">11 장-응용 프로그램 배포</span><span class="sxs-lookup"><span data-stu-id="e4b82-458">Chapter 11 - Deploy your Application</span></span>

<span data-ttu-id="e4b82-459">응용 프로그램을 배포 하려면:</span><span class="sxs-lookup"><span data-stu-id="e4b82-459">To deploy your application:</span></span>

1.  <span data-ttu-id="e4b82-460">새 Unity 빌드 ( **앱** 폴더)로 이동 하 여 **Visual Studio** 에서 솔루션 파일을 엽니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-460">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio** .</span></span>

2.  <span data-ttu-id="e4b82-461">Visual Studio를 연 상태에서 NuGet 패키지를 복원 해야 합니다 .이 작업은 솔루션 탐색기 (Visual Studio의 오른쪽에 있음)에서 MachineLearningLab_Build 솔루션을 마우스 오른쪽 단추로 클릭 한 다음 NuGet 패키지 복원을 클릭 하 여 수행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-461">With Visual Studio open, you need to Restore NuGet Packages, which can be done through right-clicking your MachineLearningLab_Build solution, from the Solution Explorer (found to the right of Visual Studio), and then clicking Restore NuGet Packages:</span></span>

    ![NuGet 패키지 추가](images/AzureLabs-Lab7-57.png)

3.  <span data-ttu-id="e4b82-463">솔루션 구성에서 **디버그** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-463">In the Solution Configuration select **Debug** .</span></span>

4.  <span data-ttu-id="e4b82-464">솔루션 플랫폼에서 **x86** , **로컬 컴퓨터** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-464">In the Solution Platform, select **x86** , **Local Machine** .</span></span> 

    > <span data-ttu-id="e4b82-465">Microsoft HoloLens의 경우 컴퓨터에 테더 링 된 하지 않도록 *원격 컴퓨터* 를 설정 하는 것이 더 쉬울 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-465">For the Microsoft HoloLens, you may find it easier to set this to *Remote Machine* , so that you are not tethered to your computer.</span></span> <span data-ttu-id="e4b82-466">그러나 다음을 수행 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-466">Though, you will need to also do the following:</span></span>
    > - <span data-ttu-id="e4b82-467">HoloLens의 **IP 주소** 를 알고 있습니다. *설정 > 네트워크 & 인터넷 > Wi-fi > 고급 옵션* 에서 찾을 수 있습니다. IPv4는 사용 해야 하는 주소입니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-467">Know the **IP Address** of your HoloLens, which can be found within the *Settings > Network & Internet > Wi-Fi > Advanced Options* ; the IPv4 is the address you should use.</span></span> 
    > - <span data-ttu-id="e4b82-468">**개발자 모드가** **설정** 되어 있는지 확인 합니다. *개발자를 위한 업데이트 & 보안 > > 설정* 에서 찾을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-468">Ensure **Developer Mode** is **On** ; found in *Settings > Update & Security > For developers* .</span></span>

    ![NuGet 패키지 추가](images/AzureLabs-Lab7-58.png)

5.  <span data-ttu-id="e4b82-470">**빌드 메뉴로** 이동 하 여 **솔루션 배포** 를 클릭 하 여 응용 프로그램을 PC에 테스트용으로 로드.</span><span class="sxs-lookup"><span data-stu-id="e4b82-470">Go to **Build menu** and click on **Deploy Solution** to sideload the application to your PC.</span></span>

6.  <span data-ttu-id="e4b82-471">이제 앱이 설치 된 앱 목록에 표시 되어 시작 될 준비가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-471">Your App should now appear in the list of installed apps, ready to be launched.</span></span>

<span data-ttu-id="e4b82-472">Mixed Reality 응용 프로그램을 실행 하는 경우 Unity 장면에 설정 된 도구를 볼 수 있으며, 초기화에서 Azure 내에 설정 된 데이터가 인출 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-472">When you run the Mixed Reality application, you will see the bench that was set up in your Unity scene, and from initialization, the data you set up within Azure will be fetched.</span></span> <span data-ttu-id="e4b82-473">데이터는 응용 프로그램 내에서 deserialize 되며, 현재 날짜 및 시간에 대 한 세 가지 상위 결과가 시각적으로 제공 됩니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-473">The data will be deserialized within your application, and the three top results for your current date and time will be provided visually, as three models on the bench.</span></span>


## <a name="your-finished-machine-learning-application"></a><span data-ttu-id="e4b82-474">완성 된 Machine Learning 응용 프로그램</span><span class="sxs-lookup"><span data-stu-id="e4b82-474">Your finished Machine Learning application</span></span>
 
<span data-ttu-id="e4b82-475">축 하 합니다. Azure Machine Learning를 활용 하 여 데이터 예측을 만들고 장면에 표시 하는 혼합 현실 앱을 빌드 했습니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-475">Congratulations, you built a mixed reality app that leverages the Azure Machine Learning to make data predictions and display it on your scene.</span></span>

![NuGet 패키지 추가](images/AzureLabs-Lab7-0.png)

## <a name="exercise"></a><span data-ttu-id="e4b82-477">연습</span><span class="sxs-lookup"><span data-stu-id="e4b82-477">Exercise</span></span>

<span data-ttu-id="e4b82-478">**연습 1**</span><span class="sxs-lookup"><span data-stu-id="e4b82-478">**Exercise 1**</span></span>

<span data-ttu-id="e4b82-479">응용 프로그램의 정렬 순서를 실험 하 고이 데이터가 유용할 수 있으므로 세 개의 하위 예측이 선반에 표시 되도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-479">Experiment with the sort order of your application and have the three bottom predictions appear on the shelf, as this data would potentially be useful also.</span></span>

<span data-ttu-id="e4b82-480">**연습 2**</span><span class="sxs-lookup"><span data-stu-id="e4b82-480">**Exercise 2**</span></span>

<span data-ttu-id="e4b82-481">**Azure 테이블** 을 사용 하 여 새 테이블에 날씨 정보를 채우고 데이터를 사용 하 여 새 실험을 만듭니다.</span><span class="sxs-lookup"><span data-stu-id="e4b82-481">Using **Azure Tables** populate a new table with weather information and create a new experiment using the data.</span></span>
