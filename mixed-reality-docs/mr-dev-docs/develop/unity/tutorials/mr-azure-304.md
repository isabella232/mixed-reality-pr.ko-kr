...
<span data-ttu-id="d3ae4-113">*Azure Face API* 는 개발자에 게 클라우드에서 가장 높은 고급 얼굴 알고리즘을 제공 하는 Microsoft 서비스입니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-113">*Azure Face API* is a Microsoft service, which provides developers with the most advanced face algorithms, all in the cloud.</span></span> <span data-ttu-id="d3ae4-114">*Face API* 에는 특성을 사용한 얼굴 감지 및 얼굴 인식 이라는 두 가지 주요 기능이 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-114">The *Face API* has two main functions: face detection with attributes, and face recognition.</span></span> <span data-ttu-id="d3ae4-115">이를 통해 개발자는 단지 얼굴에 대 한 그룹 집합을 설정 하 고 나중에 쿼리 이미지를 서비스로 보내 얼굴을 확인할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-115">This allows developers to simply set a set of groups for faces, and then, send query images to the service later, to determine to whom a face belongs.</span></span> <span data-ttu-id="d3ae4-116">자세한 내용은 [Azure 얼굴 인식 페이지](https://azure.microsoft.com/services/cognitive-services/face/)를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-116">For more information, visit the [Azure Face Recognition page](https://azure.microsoft.com/services/cognitive-services/face/).</span></span>
...
## <a name="before-you-start"></a><span data-ttu-id="d3ae4-149">시작하기 전에</span><span class="sxs-lookup"><span data-stu-id="d3ae4-149">Before you start</span></span>
...
5.  <span data-ttu-id="d3ae4-179">\**만들기 *를** 클릭 한 후에는 서비스를 만들 때까지 기다려야 합니다 .이는 1 분 정도 걸릴 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-179">Once you have clicked on **Create\*,** you will have to wait for the service to be created, this might take a minute.</span></span>
...
<span data-ttu-id="d3ae4-193">**Person Maker** 를 사용 하면 사용자와 연결 된 Azure 항목 및 사용자 그룹을 만들 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-193">**Person Maker** allows you to create Azure entries, which are associated with people, and groups of people.</span></span> <span data-ttu-id="d3ae4-194">응용 프로그램은 추가 된 사용자의 얼굴을 인식 하기 위해 나중에 FaceAPI에서 사용할 수 있는 형식으로 필요한 모든 정보를 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-194">The application will place all the needed information in a format which can then later be used by the FaceAPI, in order to recognize the faces of people whom you have added.</span></span> 
...
5.  <span data-ttu-id="d3ae4-224">삽입:</span><span class="sxs-lookup"><span data-stu-id="d3ae4-224">Insert:</span></span>
...
10. <span data-ttu-id="d3ae4-243">**Azure에 제출을** 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-243">Click on **Submit To Azure**.</span></span> <span data-ttu-id="d3ae4-244">카운터는 제출 상태를 표시 한 다음 완료 되 면 메시지를 표시 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-244">A counter will show you the state of the submission, followed by a message when it has completed.</span></span>
...
4.  <span data-ttu-id="d3ae4-262">그런 다음 **파일 > 빌드 설정** 으로 이동 하 고 플랫폼 **전환** 단추를 클릭 하 여 플랫폼을 **유니버설 Windows 플랫폼** 로 전환 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-262">Next, go to **File > Build Settings** and switch the platform to **Universal Windows Platform**, by clicking on the **Switch Platform** button.</span></span>
...
<span data-ttu-id="d3ae4-312">받아서 봇 서비스로 전송 되는 개체를 deserialize 하 고 serialize 할 수 있도록 라이브러리 *에Newtonsoft.Js* 를 다운로드 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-312">To help you deserialize and serialize objects received and sent to the Bot Service you need to download the *Newtonsoft.Json* library.</span></span> <span data-ttu-id="d3ae4-313">이 [unity 패키지 파일](https://github.com/Microsoft/HolographicAcademy/raw/Azure-MixedReality-Labs/Azure%20Mixed%20Reality%20Labs/MR%20and%20Azure%20304%20-%20Face%20recognition/newtonsoftDLL.unitypackage)에서 올바른 unity 폴더 구조를 사용 하 여 이미 구성 된 호환 버전을 찾을 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-313">You will find a compatible version already organized with the correct Unity folder structure in this [Unity package file](https://github.com/Microsoft/HolographicAcademy/raw/Azure-MixedReality-Labs/Azure%20Mixed%20Reality%20Labs/MR%20and%20Azure%20304%20-%20Face%20recognition/newtonsoftDLL.unitypackage).</span></span> 
...
## <a name="chapter-8---building-the-solution"></a><span data-ttu-id="d3ae4-365">8 장-솔루션 빌드</span><span class="sxs-lookup"><span data-stu-id="d3ae4-365">Chapter 8 - Building the solution</span></span>
...
<span data-ttu-id="d3ae4-367">이렇게 하려면 먼저 다음을 확인 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-367">Before you do, ensure that:</span></span>

-   <span data-ttu-id="d3ae4-368">3 장에서 설명한 모든 설정이 올바르게 설정 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-368">All the settings mentioned in the Chapter 3 are set correctly.</span></span> 
-   <span data-ttu-id="d3ae4-369">*FaceAnalysis* 스크립트는 주 카메라 개체에 연결 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-369">The script *FaceAnalysis* is attached to the Main Camera object.</span></span> 
-   <span data-ttu-id="d3ae4-370">*FaceAnalysis* 스크립트 내에서 **인증 키** 와 **그룹 Id** 가 모두 설정 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-370">Both the **Auth Key** and **Group Id** have been set within the *FaceAnalysis* script.</span></span>

<span data-ttu-id="d3ae4-371">이 시점에서 솔루션을 빌드할 준비가 되었습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-371">A this point you are ready to build the Solution.</span></span> <span data-ttu-id="d3ae4-372">솔루션이 구축 되 면 응용 프로그램을 배포할 준비가 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-372">Once the Solution has been built, you will be ready to deploy your application.</span></span>

<span data-ttu-id="d3ae4-373">빌드 프로세스를 시작 하려면 다음을 수행 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-373">To begin the Build process:</span></span>

1.  <span data-ttu-id="d3ae4-374">파일, 저장을 차례로 클릭 하 여 현재 장면을 저장 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-374">Save the current scene by clicking on File, Save.</span></span>
2.  <span data-ttu-id="d3ae4-375">파일, 빌드 설정으로 이동 하 여 열려 있는 장면 추가를 클릭 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-375">Go to File, Build Settings, click on Add Open Scenes.</span></span>
3.  <span data-ttu-id="d3ae4-376">Tick Unity c # 프로젝트를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-376">Make sure to tick Unity C# Projects.</span></span>

    ![Visual Studio 솔루션 배포](images/AzureLabs-Lab4-24.png)

4.  <span data-ttu-id="d3ae4-378">빌드를 누릅니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-378">Press Build.</span></span> <span data-ttu-id="d3ae4-379">이 작업을 수행 하면 Unity는 파일 탐색기 창을 시작 합니다. 여기에서 앱을 빌드할 폴더를 만들고 선택 해야 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-379">Upon doing so, Unity will launch a File Explorer window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="d3ae4-380">이제 Unity 프로젝트 내에서 해당 폴더를 만들고 it 앱을 호출 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-380">Create that folder now, within the Unity project, and call it App.</span></span> <span data-ttu-id="d3ae4-381">그런 다음 앱 폴더를 선택 하 고 폴더 선택을 누릅니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-381">Then with the App folder selected, press Select Folder.</span></span> 
5.  <span data-ttu-id="d3ae4-382">Unity는 응용 프로그램 폴더에서 프로젝트 빌드를 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-382">Unity will begin building your project, out to the App folder.</span></span> 
6.  <span data-ttu-id="d3ae4-383">Unity가 빌드를 완료 하면 (시간이 걸릴 수 있음) 빌드 위치에서 파일 탐색기 창이 열립니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-383">Once Unity has finished building (it might take some time), it will open a File Explorer window at the location of your build.</span></span>

    ![Visual Studio에서 솔루션 배포](images/AzureLabs-Lab4-25.png)

7.  <span data-ttu-id="d3ae4-385">앱 폴더를 연 다음, MR_FaceRecognition 위에 표시 된 대로 새 프로젝트 솔루션을 엽니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-385">Open your App folder, and then open the new Project Solution (as seen above, MR_FaceRecognition.sln).</span></span>


## <a name="chapter-9---deploying-your-application"></a><span data-ttu-id="d3ae4-386">9 장-응용 프로그램 배포</span><span class="sxs-lookup"><span data-stu-id="d3ae4-386">Chapter 9 - Deploying your application</span></span>

<span data-ttu-id="d3ae4-387">HoloLens에 배포 하려면:</span><span class="sxs-lookup"><span data-stu-id="d3ae4-387">To deploy on HoloLens:</span></span>

1.  <span data-ttu-id="d3ae4-388">HoloLens의 IP 주소 (원격 배포의 경우)가 필요 하 고 HoloLens가 **개발자 모드** 에 있는지 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-388">You will need the IP Address of your HoloLens (for Remote Deploy), and to ensure your HoloLens is in **Developer Mode**.</span></span> <span data-ttu-id="d3ae4-389">가상 하드 디스크 파일에 대한 중요 정보를 제공하려면</span><span class="sxs-lookup"><span data-stu-id="d3ae4-389">To do this:</span></span>

    1. <span data-ttu-id="d3ae4-390">HoloLens를 입고 하는 동안 **설정을** 엽니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-390">Whilst wearing your HoloLens, open the **Settings**.</span></span>
    2. <span data-ttu-id="d3ae4-391">**네트워크 & 인터넷 > Wi-Fi > 고급 옵션** 으로 이동 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-391">Go to **Network & Internet > Wi-Fi > Advanced Options**</span></span>
    3. <span data-ttu-id="d3ae4-392">**IPv4** 주소를 적어둡니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-392">Note the **IPv4** address.</span></span>
    4. <span data-ttu-id="d3ae4-393">그런 다음 **설정** 으로 다시 이동한 다음 **개발자를 위한 & 보안 >를 업데이트** 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-393">Next, navigate back to **Settings**, and then to **Update & Security > For Developers**</span></span> 
    5. <span data-ttu-id="d3ae4-394">에서 개발자 모드를 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-394">Set Developer Mode On.</span></span>

2.  <span data-ttu-id="d3ae4-395">새 Unity 빌드 ( *앱* 폴더)로 이동 하 여 *Visual Studio* 에서 솔루션 파일을 엽니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-395">Navigate to your new Unity build (the *App* folder) and open the solution file with *Visual Studio*.</span></span>
3.  <span data-ttu-id="d3ae4-396">솔루션 구성에서 **디버그** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-396">In the Solution Configuration select **Debug**.</span></span>
4.  <span data-ttu-id="d3ae4-397">솔루션 플랫폼에서 **x86**, **원격 컴퓨터** 를 선택 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-397">In the Solution Platform, select **x86**, **Remote Machine**.</span></span> 

    ![솔루션 구성 변경](images/AzureLabs-Lab4-26.png)
 
5.  <span data-ttu-id="d3ae4-399">**빌드 메뉴로** 이동 하 여 **솔루션 배포** 를 클릭 하 여 응용 프로그램을 HoloLens로 테스트용으로 로드.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-399">Go to the **Build menu** and click on **Deploy Solution**, to sideload the application to your HoloLens.</span></span>
6.  <span data-ttu-id="d3ae4-400">이제 앱이 HoloLens에 설치 된 앱 목록에 표시 되어 시작할 준비가 되었습니다!</span><span class="sxs-lookup"><span data-stu-id="d3ae4-400">Your App should now appear in the list of installed apps on your HoloLens, ready to be launched!</span></span>

> [!NOTE]
> <span data-ttu-id="d3ae4-401">모던 헤드셋에 배포 하려면 **솔루션 플랫폼** 을 *로컬 컴퓨터* 로 설정 하 고 **구성을** *디버그* 로 설정 하 고 *x 86* 을 **플랫폼** 으로 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-401">To deploy to immersive headset, set the **Solution Platform** to *Local Machine*, and set the **Configuration** to *Debug*, with *x86* as the **Platform**.</span></span> <span data-ttu-id="d3ae4-402">그런 다음 **빌드 메뉴** 를 사용 하 여 *솔루션 배포* 를 선택 하 여 로컬 컴퓨터에 배포 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-402">Then deploy to the local machine, using the **Build menu**, selecting *Deploy Solution*.</span></span> 


## <a name="chapter-10---using-the-application"></a><span data-ttu-id="d3ae4-403">10 장-응용 프로그램 사용</span><span class="sxs-lookup"><span data-stu-id="d3ae4-403">Chapter 10 - Using the application</span></span>

1.  <span data-ttu-id="d3ae4-404">HoloLens를 입고 앱을 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-404">Wearing the HoloLens, launch the app.</span></span>
2.  <span data-ttu-id="d3ae4-405">*Face API* 등록 한 사용자를 확인 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-405">Look at the person that you have registered with the *Face API*.</span></span> <span data-ttu-id="d3ae4-406">다음 사항을 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-406">Make sure that:</span></span>

    -  <span data-ttu-id="d3ae4-407">개인의 얼굴이 너무 멀리 떨어져 있으며 명확 하 게 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-407">The person's face is not too distant and clearly visible</span></span>
    -  <span data-ttu-id="d3ae4-408">환경 조명이 너무 어두움</span><span class="sxs-lookup"><span data-stu-id="d3ae4-408">The environment lighting is not too dark</span></span>

3.  <span data-ttu-id="d3ae4-409">탭 제스처를 사용 하 여 사용자의 사진을 캡처합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-409">Use the tap gesture to capture the person's picture.</span></span>
4.  <span data-ttu-id="d3ae4-410">앱이 분석 요청을 보내고 응답을 받을 때까지 기다립니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-410">Wait for the App to send the analysis request and receive a response.</span></span>
5.  <span data-ttu-id="d3ae4-411">사용자가 성공적으로 인식 되 면 사용자 이름이 UI 텍스트로 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-411">If the person has been successfully recognized, the person's name will appear as UI text.</span></span>
6.  <span data-ttu-id="d3ae4-412">몇 초 마다 탭 제스처를 사용 하 여 캡처 프로세스를 반복할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-412">You can repeat the capture process using the tap gesture every few seconds.</span></span>

## <a name="your-finished-azure-face-api-application"></a><span data-ttu-id="d3ae4-413">완성 된 Azure Face API 응용 프로그램</span><span class="sxs-lookup"><span data-stu-id="d3ae4-413">Your finished Azure Face API Application</span></span>

<span data-ttu-id="d3ae4-414">축 하 합니다. Azure 얼굴 인식 서비스를 활용 하 여 이미지 내 얼굴을 감지 하 고 알려진 얼굴을 식별 하는 혼합 현실 앱을 빌드 했습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-414">Congratulations, you built a mixed reality app that leverages the Azure Face Recognition service to detect faces within an image, and identify any known faces.</span></span>

![이 과정을 완료 한 결과](images/AzureLabs-Lab4-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="d3ae4-416">보너스 연습</span><span class="sxs-lookup"><span data-stu-id="d3ae4-416">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="d3ae4-417">연습 1</span><span class="sxs-lookup"><span data-stu-id="d3ae4-417">Exercise 1</span></span>

<span data-ttu-id="d3ae4-418">**Azure Face API** 는 단일 이미지에서 최대 64 얼굴을 검색할 수 있을 만큼 강력 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-418">The **Azure Face API** is powerful enough to detect up to 64 faces in a single image.</span></span> <span data-ttu-id="d3ae4-419">응용 프로그램을 확장 하 여 두 개 또는 세 개의 얼굴을 인식할 수 있도록 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-419">Extend the application, so that it could recognize two or three faces, amongst many other people.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="d3ae4-420">연습 2</span><span class="sxs-lookup"><span data-stu-id="d3ae4-420">Exercise 2</span></span>

<span data-ttu-id="d3ae4-421">또한 **Azure Face API** 는 모든 종류의 특성 정보를 다시 제공할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-421">The **Azure Face API** is also able to provide back all kinds of attribute information.</span></span> <span data-ttu-id="d3ae4-422">응용 프로그램에이를 통합 합니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-422">Integrate this into the application.</span></span> <span data-ttu-id="d3ae4-423">이는 [Emotion API](https://azure.microsoft.com/services/cognitive-services/emotion/)와 결합 될 때 훨씬 더 흥미롭습니다.</span><span class="sxs-lookup"><span data-stu-id="d3ae4-423">This could be even more interesting, when combined with the [Emotion API](https://azure.microsoft.com/services/cognitive-services/emotion/).</span></span>