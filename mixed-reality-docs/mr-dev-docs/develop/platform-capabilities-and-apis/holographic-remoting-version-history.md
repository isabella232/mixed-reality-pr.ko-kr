...
* OpenXR API를 사용하는 홀로그램 Remoting은 이제 다음을 지원합니다.
  ...
  * 애플리케이션이 XrSpace 핸들을 만들어 다른 Windows Mixed Reality 디바이스 플랫폼 라이브러리 또는 API의 공간 Graph 노드를 추적할 수 있도록 하는 XR_MSFT_spatial_graph_bridge 확장입니다. 그러나 주의할 점은 XR_SPATIAL_GRAPH_NODE_TYPE_STATIC_MSFT xrCreateSpatialGraphNodeSpaceMSFT에서 지원하는 유일한 노드 유형입니다. 
...
* OpenXR 및 Mixed Reality API를 모두 사용하는 홀로그램 Remoting은 이제 다음을 지원합니다.
  * Microsoft.MixedReality.SceneUnderstanding SDK를 사용하면 애플리케이션이 쿼드, 메시 및 콘텐츠 배치 큐를 제공하는 사용자(예: 벽, 바닥 및 표면)를 둘러싼 장면에 대한 설명을 계산할 수 있습니다.
...
* [OpenXR API를](../native/openxr.md) 사용하는 홀로그램 Remoting은 이제 다음을 지원합니다.
  * 확장 XR_MSFT_spatial_anchor. 이 확장을 사용하면 애플리케이션이 런타임에서 추적할 사용자의 물리적 환경에서 임의의 프리스페이스 지점인 공간 앵커를 만들 수 있습니다.
  * XR_MSFT_controller_model 확장입니다. 이 확장은 컨트롤러에 대한 GLTF 모델을 로드하는 메커니즘을 제공합니다.
...
* Microsoft Store 통해 사용할 수 있는 홀로그램 Remoting 플레이어의 성능이 향상되었습니다. HoloLens 2 플레이어가 이제 초당 60프레임에서 단색으로 실행됩니다.
...
* 데스크톱의 홀로그램 원격 플레이어: Windows Mixed Reality 자동으로 닫는 대신 설치되지 않은 경우 오류 메시지를 표시합니다.
...
* RemoteContext.Config ureDepthVideoStream을 통해 깊이 버퍼 스트리밍을 사용하지 않도록 설정하는 데 사용할 수 있는 DepthBufferStreamResolution에 사용 안 함을 추가했습니다. 사용된 경우 [HolographicCameraRenderingParameters.CommitDirect3D11DepthBuffer는](/uwp/api/windows.graphics.holographic.holographiccamerarenderingparameters.commitdirect3d11depthbuffer) *E_ILLEGAL_METHOD_CALL* 함께 실패합니다.
...
* 2.1.0보다 작은 버전을 사용하여 최신 홀로그램 원격 플레이어와 원격 앱 간의 오디오 이전 버전과의 호환성 문제를 해결했습니다.
...
* VR 컨트롤러의 트리거 단추를 사용할 때의 충돌을 해결했습니다. 홀로그램 Remoting은 컨트롤러를 완전히 지원하지 않으며, 트리거 단추와 Windows 단추만 HoloLens 2 쌍을 이루는 경우 작동합니다.
