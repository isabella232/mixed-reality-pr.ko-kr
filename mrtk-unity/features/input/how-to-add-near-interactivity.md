...
---
# <a name="how-to-add-near-interactivity"></a>근접 대화형 작업을 추가 하는 방법
...
기본 mrtk 프로필 및 기본 HoloLens 2 프로필에는 이미 *pokepointer* 가 포함 되어 있습니다. Mrtk 구성 프로필을 선택 하 고 **입력**  >  **포인터**  >  **포인터 옵션** 으로 이동 하 여 pokepointer가 생성 되는지 확인할 수 있습니다. 기본 `PokePointer` (asset/MRTK/SDK/Features/UX/Prefabs/포인터) prefab는 *컨트롤러 유형의* *트레일러* 표시와 함께 나열 되어야 합니다. 사용자 지정 prefab는 클래스를 구현 하는 동안 활용할 수 있습니다 [`PokePointer`](xref:Microsoft.MixedReality.Toolkit.Input.PokePointer) .
...
1. Touchable GameObject에서 BoxCollider and a [ `NearInteractionTouchable` ] (f: Toolkit MixedReality를 추가 합니다. NearInteractionTouchable) 구성 요소를 입력 합니다.

    1. [   `IMixedRealityTouchHandler` ] (F: MixedReality. Toolkit을 사용 하는 경우 터치에 대 한 이벤트를 설정 합니다. IMixedRealityTouchHandler)를 구성 요소 스크립트에서 인터페이스에 입력 합니다.
...
1. 해당 개체 또는 해당 상위 항목 중 하나에서를 구현 하는 스크립트 구성 요소를 추가 합니다. [`IMixedRealityTouchHandler`](xref:Microsoft.MixedReality.Toolkit.Input.IMixedRealityTouchHandler)
   인터페이스입니다. [ `NearInteractionTouchable` ] (F: MixedReality. Toolkit를 사용 하는 개체의 상위 항목입니다. NearInteractionTouchable)도 포인터 이벤트를 받을 수 있습니다.
...
1. Touchable GameObject에서 collider 및 a [ `NearInteractionTouchableVolume` ] (f: Toolkit MixedReality를 추가 합니다. NearInteractionTouchableVolume) 구성 요소를 입력 합니다.
